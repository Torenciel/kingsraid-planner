...
export const GearSetProvider = ({ children }) => {
  ...
  const getGearSetImageUrl = useCallback((gearSet) => {
    if (!gearSet) return '';
  ...
  const getGearSetBySlug = useCallback((slug) => {
    if (!slug) return null;
  ...
  const getGearSetById = useCallback((id) => {
    if (!id) return null;
  ...
  const searchGearSets = useCallback((searchTerm) => {
  ...
  const createSingleGearSetForSave = useCallback((gearSet, pieces = 4) => {
  ...
  const createMultiGearSetForSave = useCallback((gearSet1, gearSet2) => {
  ...
  const createGearSetForSave = useCallback((selectedSets) => {
  ...
  const convertDBGearSetToFrontend = useCallback((dbGearSet) => {
    if (!dbGearSet) return null;
    
    // Vérifier si c'est un multi-set
    if (dbGearSet.isMultiSet && dbGearSet.sets && Array.isArray(dbGearSet.sets)) {
      ...
      return {
        ...
        pieces: 2,
        set1Info: dbGearSet.set1Info || (set1 ? {
          name: set1.name,
          slug: set1.slug,
          bonus2P: set1.bonus2P
        } : null),
        set2Info: dbGearSet.set2Info || (set2 ? {
          name: set2.name,
          slug: set2.slug,
    ...
    return {
      ...
      gearSetInfo: dbGearSet.gearSetInfo || (gearSet ? {
        name: gearSet.name,
        thumbnail: getGearSetImageUrl(gearSet),
        bonus2P: gearSet.bonus2P,
        bonus4P: gearSet.bonus4P
  ...
  const getGearSetNames = useCallback((gearSetData) => {
