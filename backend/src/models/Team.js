...
const TeamSchema = new mongoose.Schema(
  {
    ...
    // References the User document in the database
    author: {
      type: mongoose.Schema.Types.ObjectId,
      ref: "User",
      required: true,
    ...
    // Author name stored separately since it can change
    createdBy: {
      type: String,
      required: true,
      index: true,
  ...
  if (this.heroes && this.isModified("heroes")) {
    ...
    for (const heroConfig of this.heroes) {
      if (heroConfig.heroSlug && !heroConfig.heroId) {
        const hero = await Hero.findOne({ slug: heroConfig.heroSlug }).select(
          "_id",
        );
